...
describe("Exploratory tests - string", function() {
  ...
  it("Pad all strings to size of longest", function() {
    ...
    for (let i = 0; i < strings.length; i++){
      if (strings[i].length > dlzka) dlzka = strings[i].length;
    }

    for (let i = 0; i < strings.length; i++){
      strings[i] = strings[i].padStart(dlzka, ' ');
    }
    assert.deepStrictEqual(strings, ["  a", " bb", "ccc"]);
  });
  it("replace all animals in sentence", function() {
    const sentence = "cats ignore dogs";
    let veta = sentence;
    veta = veta.replace(/cats|dogs/g, '');
